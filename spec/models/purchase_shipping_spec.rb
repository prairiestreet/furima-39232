...
RSpec.describe PurchaseShipping, type: :model do
 before do
  ...
  @purchase_shipping = FactoryBot.build(:purchase_shipping, user_id: @user.id, item_id: @item.id, address: "テスト住所")
  end
  ...
  describe '購入情報と発送先情報を保存' do
    ...
    context '内容に問題がある場合' do
      ...
      it "郵便番号が「3桁ハイフン4桁」の半角文字列でないと登録できない" do
        @purchase_shipping.postal_code = '11-2222'
        @purchase_shipping.valid?
      ...
      it "電話番号は10桁以上でないと登録できない" do
        ...
        @purchase_shipping.valid?
        expect(@purchase_shipping.errors.full_messages).to include('Phone number must be 10 digits or 11 digits number, not full width')
      end
      ...
      it "電話番号は半角数値のみでないと登録できない" do
        @purchase_shipping.phone_number = '123-456-7890'
        @purchase_shipping.valid?
        expect(@purchase_shipping.errors.full_messages).to include('Phone number must be 10 digits or 11 digits number, not full width')
      end
